...
from JaxSC2.env.twobridge import TwoBridgeEnv, CentralAction, get_action_masks
from algorithms.mask_ppo.model import MaskedActorCritic
from algorithms.mask_ppo.ppo_logic import masked_ppo_loss
...
from algorithms.mask_ppo.eval import evaluate

class MaskedPPOTrainer:
    """Masked Proximal Policy Optimization trainer for action-constrained environments.

    This trainer is a specialized version of PPO that handles invalid-action 
    masking at the logic level. It uses a `MaskedActorCritic` model and 
    `masked_ppo_loss` to ensure the policy only explores legal action spaces.

    Attributes:
        config (dict): Configuration dictionary including masking-specific params.
    """
    ...
    def train(self, total_steps: int):
        """Trains the agent with strict action-masking enforcement.

        Identical to standard PPO but integrates valid-action masks into the 
        rollout collection and optimization phases. This ensures that the 
        agent never attempts invalid actions during training or exploration.

        Args:
            total_steps (int): Total environmental steps for training.
        """
        config = self.config
        ...
        print("="*50)
        print(f"JaxSC2 Masked PPO Trainer | Variant: {config['VARIANT_NAME']}")
        print("="*50)
        
        env = TwoBridgeEnv(variant_name=config["VARIANT_NAME"], use_spatial_obs=False)
        logger = Logger(f"runs/{config['VARIANT_NAME']}_masked_ppo/logs")
        ckpt_dir = f"runs/{config['VARIANT_NAME']}_masked_ppo/checkpoints"
        
        rng = jax.random.PRNGKey(42)
        rng, init_rng = jax.random.split(rng)
        
        dummy_obs = jnp.zeros((1, 63))
        dummy_mask = jnp.ones((1, 17), dtype=bool)
        model = MaskedActorCritic(action_dim=17)
        params = model.init(init_rng, dummy_obs, dummy_mask)
        ...
        def rollout_step(carry, _):
            ...
            masks = jax.vmap(lambda s: get_action_masks(s, env.num_allies))(env_state)
            logits, value = model.apply(params, obs, masks)
            rng, sub_rng = jax.random.split(rng)
            action_idx = jax.random.categorical(sub_rng, logits)
            logp = jnp.take_along_axis(jax.nn.log_softmax(logits), action_idx[:, None], axis=1).squeeze(-1)
            ...
            transition = (obs, action_idx, logp, value, reward, done, masks)
            return (params, next_env_state, next_obs_flat, rng), transition

        def update_minibatch(train_state, batch):
            obs, act, logp, adv, ret, masks = batch
            (loss, aux), grads = jax.value_and_grad(masked_ppo_loss, has_aux=True)(
        ...
        def train_iteration(state, env_state, obs, rng, rms):
            ...
            obs_b, act_b, logp_b, val_b, rew_b, done_b, mask_b = transitions
            
            rms = update_rms(rms, rew_b.flatten())
            norm_rew_b = rew_b / jnp.sqrt(rms.var + 1e-8)
            mean_reward = jnp.mean(rew_b)
            
            _, last_val = model.apply(params, next_obs, jnp.ones((num_envs, 17), dtype=bool)) 
            ...
            batch = (obs_b, act_b, logp_b, adv_b, ret_b, mask_b)
